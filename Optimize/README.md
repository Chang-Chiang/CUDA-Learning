...
- 编译命令:
    - `nvcc -O3 -arch=sm_61 xxx.cu`
    - 使用宏 USE_DP:`vcc -O3 -arch=sm_61 -DUSE_DP xxx.cu `
...
| ----------------------------------- | ------------------------------------------------------------ |
| 数据量                              | 100 000 000                                                  |
| grid_size，多少个 block             | (N - 1) / block_size + 1 = 781250<br />(N + block_size - 1) / block_size = 781250 |
| block_size，每个 block 有多少个线程 | 128                                                          |
...
1. 首先分别统计 cpu 和 gpu 下耗时
    ...
    | ------ | ------- | ------- |
    | float  | 76.6ms  | 7.74ms  |
    | double | 172.7ms | 15.39ms |

2. 重叠主机、设备计算

    根据 1 可知，相同数据量下，cpu 计算耗时是 gpu 的 10 倍，考虑通过 重叠主机、设备计算缩短耗时，即：
